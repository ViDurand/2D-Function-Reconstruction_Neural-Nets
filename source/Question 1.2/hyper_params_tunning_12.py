...
def objective(params, k, data):
    ...
    best_params = {'N': 0, 'sigma': 0, 'rho': 0, 'best_error': 1000}
    for N in Ns:
        for sigma in sigmas:
            for rho in rhos:
                print()
                ...
                print("sigma = ", sigma)
                print("rho = ", rho)
                RBF = RBFNetwork(hidden_layer_sizes = N, solver = 'bfgs', sigma = sigma, rho = rho, random_state = 1792126)    
                kf = KFold(n_splits=k)
                sum_error = 0
                for train, test in kf.split(data):
                    ...
                    sum_error += RBF.get_error(result_output_test, test_set_outputs)
